...
さらに 1日1回・引け後（JST 16:35）に、**材料ニュースが無くても教材条件を満点で満たした銘柄**を日経225から拾って通知する（→ [教材条件スキャン](#教材条件スキャン材料ニュース無しの通知枠)）。同じタイミングで、その日の稼働結果を伝える[日次サマリ](#日次サマリ)も届く。
...
■ 6501 Hitachi, Ltd.
 判定 教材条件を満点で充足（材料ニュースなし）
 株価 4,120円（前日比 +0.8%）→ エントリー候補
 25MA 4,050 ／ 75MA 3,890 ／ RSI 58 ／ 出来高 1.1倍
 ステージ 追随期（押し目反発を確認・水平ロールリバーサル成立）
 週足 上昇トレンド ／ 25MA 上向き
...
`--scan-market`。TDnet に材料が出ていなくても、**[教材由来の追加判定](#教材由来の追加判定)の5項目すべて**を満たした銘柄を通知する。1日1回・引け後（JST 16:35）にだけ走る（教材条件は週足ピボットと日足の押し目で決まるので、日足が確定する前に何度も回しても結果は変わらない）。

- **対象**: 日経225（`nikkei225.py` のスナップショット。`state/universe.txt` に1行1コードで書けばそちらが優先され、監視したい銘柄だけに絞れる）
- **出来高条件は課さない**（`SCAN_REQUIRE_VOLUME = False`）。材料の出ていない銘柄の出来高が平常なのは当たり前で、そもそも教材の「追随期＝押し目からの反発」は出来高が細る局面。RSI・ギャップ・25MA>75MA のガードは効かせる
- **地合いが「良好」でないと1件も出ない**。地合いは5項目のうちの1つなので、日経平均が 25MA を割っている間はこの枠の通知は止まる（教材: 指数の天井サイン中は新規買いを止める）
- 一度通知した銘柄は `SCAN_COOLDOWN_DAYS`（既定3日）のあいだ再通知しない。同じ条件が数日続いても毎日は鳴らない
- 5項目すべて＋チャートのガードを同時に満たすのは**かなり厳しい条件**なので、0件の日が続くのが普通。鳴ったときは教材の言う「唯一の買い場」に該当している
...
一方この5項目を**満点で満たすこと自体**が、材料ニュース無しで通知する[教材条件スキャン](#教材条件スキャン材料ニュース無しの通知枠)の条件になる。
